...
def generate_launch_description():
    ...
    path = os.path.join(package_dir,'cfg','1843_best_range_res.cfg')
    device = "1843"
    name = "/mmWaveCLI"
    ...
    gui_args = DeclareLaunchArgument(
        name="start_rviz",
        default_value="false",
        description="Launch Rviz2 runtime"
    )

    tf_args = DeclareLaunchArgument(
        name="publish_tf",
        default_value="false",
        description="Publisher Static Transform"
    )
    ...
    UDPBridge = Node(
        package="ti_mmwave_rospkg",
        executable="PointCloudUDPBridge",
        name="PointCloudUDPBridge",
        output="screen",
        emulate_tty=True,
    ...
    ld.add_action(gui_args)
    ld.add_action(tf_args)
